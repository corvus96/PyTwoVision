...
class BuildYoloV3Tiny(tf.keras.Model):
    ...
    def call(self, input_shape):
        ...
        x = Conv2dBNLeakyReluLayer('conv_layer_yolo_tiny_', (1, 1, 1024, 256), postfix='0')(x)

        conv_lobj_branch = Conv2dBNLeakyReluLayer('conv_lobj_branch', (3, 3, 256, 512), postfix='')(x)
        # conv_lbbox is used to predict large-sized objects , Shape = [None, 13, 13, 255] (if num_class == 80 like coco dataset) 
        conv_lbbox = Conv2dBNLeakyReluLayer('Convolutional_large_bounding_box_layer', (1, 1, 512, 3*(self.num_class + 5)), 
                        activate=False, bn=False, postfix='')(conv_lobj_branch)
                        
        x = Conv2dBNLeakyReluLayer('intermediate_conv_layer_', (1, 1, 256, 128), postfix='A')(x)
        # upsample here uses the nearest neighbor interpolation method, which has the advantage that the
        # upsampling process does not need to learn, thereby reducing the network parameter  
        x = UpsampleLayer()(x)
        x = tf.concat([x, route_1], axis=-1)

        conv_mobj_branch = Conv2dBNLeakyReluLayer('conv_mobj_branch', (3, 3, 128, 256), postfix='')(x)
        # conv_mbbox is used to predict medium size objects, shape = [None, 26, 26, 255] (if num_class == 80 like coco dataset) 
        conv_mbbox = Conv2dBNLeakyReluLayer('Convolutional_medium_bounding_box_layer', (1, 1, 256, 3*(self.num_class + 5)),
