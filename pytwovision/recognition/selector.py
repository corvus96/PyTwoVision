...
class Recognizer:
    ...
    def train_using_weights(self, train_annotations_path, test_annotations_path, class_file_name, weights_path,
        ...
        Arguments:
            ...
            weights_path: a path which in case if it's an url with weights like: 
            'https://pjreddie.com/media/files/yolov3.weights' or 'https://pjreddie.com/media/files/yolov3-tiny.weights'
            this method first download the weights and then train the net
            but if the path is a local file the method is going to load the weights and next train the network.
