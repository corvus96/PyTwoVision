...
def parse_anno(annotation_path, xml_path, target_size=None):
    anno = open(annotation_path, 'r')
    result = []
    for line in anno:
        s = line.strip().split(' ')
        image_title = os.path.split(s[0])
        image_xml = os.path.join(xml_path, str(image_title[1][0:-4]) + ".xml")
        tree = ET.parse(open(image_xml))
        root = tree.getroot()
        for i, size in enumerate(root.iter('size')):
...
if __name__ == '__main__':
    ...
    target_size = [416, 416]
    annotation_path = "tests/test_dataset/experimental_annotations/train.txt"
    anno_result = parse_anno(annotation_path, "tests/test_dataset/VOCdevkit/VOC2012/Annotations", target_size=target_size)
